...
Do not attach API keys, `connections.json`, logs that may contain secrets, or `~/.dlc` dumps.

## Known limitations

- **Early preview.** This is not production software. Do not use it for critical tasks.
- **API keys at rest.** Provider keys are stored in `~/.dlc/connections.json` on the local machine. Anyone with access to that file can use the keys.
